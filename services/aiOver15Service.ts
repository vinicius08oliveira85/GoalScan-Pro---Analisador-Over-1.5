...
async function buildPrompt(
  ...
  return [
    ...
    '## Análise Cruzada (Combinação de Tabelas)',
    '## Sinais a Favor',
    '## Red Flags',
    '## Plano de Entrada',
    '## Sugestão Pós-Análise',
    '',
    ...
    '',
    '### ANÁLISE POR TABELA (INSTRUÇÕES DETALHADAS):',
    '',
    '⚠️ OBRIGATÓRIO: Analise cada tabela individualmente e depois combine os insights.',
    '',
    '#### Tabela GERAL (championshipTables.geral):',
    '- Rk (Ranking): Times no topo (Rk baixo) tendem a ser mais ofensivos',
    '  * Exemplo: Se home.Rk = 3 e away.Rk = 15, o time da casa tem vantagem ofensiva',
    '- GF/MP (Gols Marcados por Partida): Compare com stats.avgScored para validar consistência',
    '  * Se GF/MP ≈ stats.avgScored, dados são consistentes',
    '  * Se GF/MP > stats.avgScored, time está melhorando ofensivamente',
    '- GA/MP (Gols Sofridos por Partida): Compare com stats.avgConceded',
    '- xG vs GF: Se xG > GF, time pode estar com azar (probabilidade pode aumentar)',
    '- xGA vs GA: Se xGA < GA, defesa pode estar em crise (probabilidade de gols contra aumenta)',
    '- GD (Saldo de Gols): GD positivo = time ofensivo, aumenta probabilidade de Over',
    '- Last 5: Analise forma recente (W = vitória, D = empate, L = derrota)',
    '  * Muitos W = boa forma ofensiva',
    '  * Muitos L = problemas ofensivos ou defensivos',
    '',
    '#### Tabela STANDARD_FOR (championshipTables.standard_for):',
    '- npxG+xAG/90: Qualidade ofensiva esperada (sem pênaltis)',
    '  * Compare home.npxG+xAG/90 vs away.npxG+xAG/90',
    '  * Compare cada um com competitionAvg.npxGxAGPer90 para ver se estão acima/média/abaixo',
    '  * Times acima da média = maior probabilidade de gols',
    '- Poss (Posse de Bola): Times com mais posse criam mais chances',
    '  * Se home.Poss > 60% e away.Poss < 40%, time da casa domina = mais chances de gol',
    '- PrgP/90 (Passes Progressivos): Indica ritmo ofensivo',
    '  * PrgP alto = time joga mais no ataque = mais chances de gol',
    '- PrgC/90 (Carregadas Progressivas): Indica velocidade de transição',
    '  * PrgC alto = contra-ataques rápidos = mais chances de gol',
    '',
    '#### Tabela PASSING_FOR (championshipTables.passing_for):',
    '- Prog/90 (Passes Progressivos por 90min): Capacidade de avançar com passes',
    '  * Compare com competitionAvg.progPer90',
    '  * Prog alto = time constrói bem o jogo = mais chances de gol',
    '- KP/90 (Key Passes por 90min): Passes que geram chances de gol',
    '  * KP alto = time cria muitas chances = maior probabilidade de gol',
    '  * Compare home.KP/90 vs away.KP/90 e ambos vs competitionAvg.kpPer90',
    '- PPA/90 (Passes para Área por 90min): Passes que chegam na área',
    '  * PPA alto = time chega muito na área = mais chances de finalização e gol',
    '  * Combine com GCA para avaliar eficiência (PPA alto + GCA alto = time eficiente)',
    '',
    '#### Tabela GCA_FOR (championshipTables.gca_for):',
    '- SCA/90 (Shot-Creating Actions por 90min): Ações que geram finalizações',
    '  * SCA alto = time cria muitas finalizações = maior chance de gol',
    '  * Compare com competitionAvg.scaPer90',
    '- GCA/90 (Goal-Creating Actions por 90min): Ações que geram gols',
    '  * GCA alto = time converte chances em gols = maior probabilidade de gol',
    '  * Combine SCA e GCA: Se SCA alto mas GCA baixo = time cria mas não converte (pode melhorar)',
    '  * Se SCA alto e GCA alto = time eficiente = alta probabilidade de gol',
    '',
    '### COMBINAÇÃO DE TABELAS (ANÁLISE CRUZADA):',
    '',
    '⚠️ OBRIGATÓRIO: Combine insights de múltiplas tabelas para análise mais profunda.',
    '',
    '1. **Análise de Criação Ofensiva Completa:**',
    '   - Combine Prog/KP/PPA (passing_for) + SCA/GCA (gca_for)',
    '   - Se passing_for mostra alta criação E gca_for mostra alta conversão = probabilidade muito alta',
    '   - Se passing_for mostra alta criação MAS gca_for mostra baixa conversão = probabilidade média (time pode melhorar)',
    '',
    '2. **Análise de Qualidade vs Quantidade:**',
    '   - Combine npxG+xAG/90 (standard_for) + GF/MP (geral)',
    '   - Se npxG+xAG/90 alto MAS GF/MP baixo = time com azar, probabilidade pode aumentar',
    '   - Se npxG+xAG/90 baixo MAS GF/MP alto = time com sorte, probabilidade pode diminuir',
    '',
    '3. **Análise de Ritmo e Eficiência:**',
    '   - Combine PrgP/PrgC (standard_for) + SCA/GCA (gca_for)',
    '   - Ritmo alto (PrgP/PrgC alto) + Eficiência alta (GCA alto) = probabilidade muito alta',
    '',
    '4. **Análise de Consistência:**',
    '   - Compare stats (últimos 10 jogos) com tabelas (temporada completa)',
    '   - Se stats.avgScored ≈ GF/MP = dados consistentes, alta confiança',
    '   - Se stats.avgScored > GF/MP = forma recente melhor, dar mais peso a stats',
    '',
    '### SUGESTÃO PÓS-ANÁLISE (OBRIGATÓRIA):',
    '',
    '⚠️ Você DEVE incluir uma seção "## Sugestão Pós-Análise" no final do relatório com:',
    '',
    '1. **Recomendação de Stake (Tamanho da Aposta):**',
    '   - Baseado na confiança (IA) e EV calculado',
    '   - Se confiança >= 80% e EV > 5%: Stake médio-alto (3-5% do bankroll)',
    '   - Se confiança >= 70% e EV > 0%: Stake médio (2-3% do bankroll)',
    '   - Se confiança < 70% ou EV < 0%: Stake baixo (1% do bankroll) ou evitar',
    '',
    '2. **Monitoramento Durante a Partida (se liveData.minute > 0):**',
    '   - Se partida está ao vivo, forneça estratégias de monitoramento',
    '   - Exemplo: "Se 0-0 aos 30min, probabilidade cai para X%. Considere hedge se odd subir."',
    '   - Exemplo: "Se 1-0 aos 20min, probabilidade aumenta para X%. Mantenha aposta."',
    '',
    '3. **Alertas de Risco Específicos:**',
    '   - Baseado nas 4 tabelas analisadas, identifique riscos específicos',
    '   - Exemplo: "Time visitante tem GCA baixo - pode ter dificuldade para marcar"',
    '   - Exemplo: "Ambos times têm xG alto mas GF baixo - pode ser jogo de poucos gols"',
    '',
    '4. **Estratégias de Hedge (se aplicável):**',
    '   - Se partida está ao vivo e cenário mudou, sugira estratégias de hedge',
    '   - Exemplo: "Se 0-0 aos 60min, considere hedge em Under 1.5 para proteger aposta"',
    '   - Exemplo: "Se 1-0 aos 45min, probabilidade ainda alta - mantenha aposta"',
    '',
    '5. **Próximos Passos:**',
    '   - Sugestões de quando revisar a análise',
    '   - Sugestões de dados adicionais a coletar para próxima análise',
    '   - Avisos sobre fatores que podem mudar a análise (lesões, escalações, etc.)'
  ].join('\n');
}
