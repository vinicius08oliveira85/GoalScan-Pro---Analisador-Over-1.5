...
async function main() {
  ...
  console.log('Este script irá ajudá-lo a configurar as variáveis de ambiente do Supabase no Vercel.\n');
  ...
  const useCLI = await question('Deseja tentar configurar automaticamente via CLI do Vercel? (s/n): ');
    ...
    try {
      ...
      if (runNow.toLowerCase() === 's' || runNow.toLowerCase() === 'sim') {
        ...
        try {
          ...
          const proc1 = require('child_process').spawn('vercel', [
            'env', 'add', 'VITE_SUPABASE_URL',
          ...
          proc1.on('close', (code) => {
            if (code === 0) {
              ...
              const proc2 = require('child_process').spawn('vercel', [
